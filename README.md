...
src/
  ...
  components/
    ...
    events/AlienArt.tsx -> as cenas da Invasão Alienígena: frota de discos voadores, Imperador Zorg, raio trator, Cristais de Energia, Escudo Arcano
  app/
...
- 🎃 Eventos da Academia — evento de Halloween "A Noite do Bug Assombrado":
  - **Salão dos Eventos** (Eventos, o primeiro item do menu da Academia, antes de Missões): banner animado e um card grande por evento, com pôster animado (castelo sob a lua de sangue, morcegos, o Rei Abóbora espiando), selo de status (Novo / Em andamento x/y / Concluído), resumo da história, progresso em Lanternas Sagradas e a recompensa final. Botão **Entrar no evento**
...
- 📅 Iniciar e encerrar eventos:
  - Cada evento tem uma situação por professor: ⏸ Não iniciado, 🟢 Acontecendo ou ⏹ Encerrado. Os alunos só veem no Salão dos Eventos (e só conseguem abrir a tela do evento) o que o professor deles deixou acontecendo; sem nenhum, o Salão mostra "Nenhum evento acontecendo agora"
  - No card de cada evento (Painel do Mestre) há **▶ Iniciar evento** e **⏹ Encerrar evento** (com confirmação); um evento encerrado pode ser reaberto com **▶ Reabrir evento**. Encerrar só esconde: o progresso dos alunos (abertura vista, missões, final) fica guardado e volta quando o evento é reaberto. Se o evento ainda não tiver missões, o card avisa antes de iniciar
...
- 🛸 Evento Invasão Alienígena "A Invasão de Bugzar" (terceiro card do Salão dos Eventos):
  ...
  - **Final** (4 cenas): os cristais carregados e a cúpula do Escudo Arcano subindo sobre o castelo, o Imperador recortado pelo próprio Ctrl+X até virar um alienzinho de coroa torta, a frota fugindo ao amanhecer enquanto os alunos abduzidos descem sãos e salvos (e um discozinho fica pra trás), e a recompensa: 👽 Alienzinho de Bugzar (mascote lendário equipável), +500 XP e +300 moedas
  - **Missões prontas** pro professor: Decifrando o Sinal (strings e template strings), O Idioma dos Números (%, Math.max, '2' + 2), A Cabine de Comando (ternário, switch, ??) e O Protocolo do Escudo (classes, JSON e desestruturação)
  - Ajuste que vale pros três eventos: elementos centralizados com translate e animados com transform (a aboborinha, o zumbizinho e o alienzinho aparecendo, a nuvem de fumaça, a cúpula) agora centralizam com margem automática ou numa camada de fora, porque o transform da animação apagava o centro e eles apareciam deslocados pra direita
